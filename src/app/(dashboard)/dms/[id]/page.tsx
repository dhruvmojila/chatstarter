...
function MessageActions({ message }: { message: Message }) {
  ...
  const rempveMutation = useMutation(api.functions.message.remove);
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent>
        <DropdownMenuItem
          ...
          onClick={() => rempveMutation({ id: message._id })}
        >
          <TrashIcon />
          Delete
        </DropdownMenuItem>
...
function MessageInput({
  ...
  const setTypingIndicator = useMutation(api.functions.typing.upsert);
  ...
  return (
    <form className="flex items-center p-4 gap-2" onSubmit={handleSubmit}>
      <Input
        ...
        onKeyDown={(e) => {
          if (content.length > 0) {
            setTypingIndicator({ directMessage });
          }
        }}
      />
